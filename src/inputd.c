...
//             char character = 'A' ;//keyToChar(key);
...
//             (position > 0 && handler->buffer[position - 1].type != EMPTY_CHARACTER && handler->buffer[position - 1].type != CURSOR_CHARACTER && (handler->buffer[position].type == EMPTY_CHARACTER || handler->buffer[position].type == CURSOR_CHARACTER))
//         ) &&
...
//     int furthestVisualLeftX = handler->buffer[0].x + offsetX;
//     if(furthestVisualLeftX > handler->maxBounds->x) {
//         offsetX -= furthestVisualLeftX - handler->maxBounds->x; //take away the difference from the visual x to the begining of input to align the input to begining
//     }
